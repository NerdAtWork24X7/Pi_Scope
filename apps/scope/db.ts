/**
 * db.ts — SQLite schema + prepared queries for Pi Scope server.
 *
...
import type { ObsEvent, SessionSummary } from "../../shared/types.ts";
...
export function prepare(db: DatabaseSync): PreparedQueries {
  ...
  const getSessionStats = db.prepare(`
    SELECT
      ...
      COALESCE(SUM(CASE WHEN type = 'error' OR (type = 'tool_result' AND (json_extract(payload_json, '$.is_error') = 1 OR (json_extract(payload_json, '$.details_summary.exit_code') IS NOT NULL AND json_extract(payload_json, '$.details_summary.exit_code') != 0))) THEN 1 ELSE 0 END), 0) AS error_count
    FROM events
    WHERE session_id = $session_id
  `);
